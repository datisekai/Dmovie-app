...
const movie = {
  ...
  getNowPlaying: async (page = 1) => {
    const dataNowPlay = await axiosClient.get(
      `/movie/now_playing?api_key=${API_KEY}&page=${page}&language=vi`
    );
    return dataNowPlay.data;
  },
  getMovieById: async (id: string) => {
    const data = await axiosClient.get(
      `/movie/${id}?api_key=${API_KEY}&language=vi`
    );
    return data.data;
  },
  getSimilarMovie: async (id: string) => {
    const data = await axiosClient.get(
      `/movie/${id}/similar?api_key=${API_KEY}&page=1&language=vi`
    );
    return data.data;
  },
